...
> RTOS가 실시간성을 *보장한다*는 말은
> **“최악의 경우 시간(Worst Case)을 계산할 수 있다”**는 뜻이야.

---

### 일반 OS는 왜 ‘보장 못 한다’고 하나?

일반 OS (Linux, Windows)는:

* 태스크 개수 가변
* 우선순위 정책 복잡
* 캐시, 가상메모리, 백그라운드 작업 개입
그래서 이런 질문에 답을 못 해:

* “이 인터럽트는 **최대 몇 µs 후에** 처리될까?”
* “이 태스크는 **최대 언제 실행 시작될까?**”

평균은 빠르지만 **최악의 경우가 계산 불가**

---

### RTOS는 뭘 해서 ‘알 수 있게’ 만드나?
...
“우선순위 높은 놈은 무조건 먼저 실행”

e.g.

* 인터럽트 처리: 최대 2µs
* 최고 우선순위 태스크: 인터럽트 끝나면 바로 실행

→ **상한선 계산 가능**

---

#### (2) 커널 함수 실행 시간이 정해져 있음

RTOS 커널은:

* 동적 자료구조 거의 없음
* 함수 실행 시간 O(1) 또는 매우 작음

예:

* context switch: 최대 5µs
* semaphore give/take: 최대 3µs

커널 문서에 **최대 실행 시간**이 나와 있음
e.g. FreeRTOS

---

### 그래서 실제로는 이렇게 “계산”함 (중요)

### 예시 상황

* 태스크 A: 우선순위 3 (주기 10ms)
* 태스크 B: 우선순위 2
* 태스크 C: 우선순위 1
* 인터럽트 처리: 최대 2µs
...
1. 인터럽트 최대 지연: 2µs
2. 더 높은 우선순위 태스크 B 실행 시간: 3ms
3. context switch: 5µs
...
* while(1)에서 안 빠져나오는 태스크
* 너무 긴 인터럽트
* malloc/free 남발
* 우선순위 설계 개판

그래서 실시간성은 **RTOS + 설계 실력**의 결과.

---

### 현업에서 “보장한다”고 말하는 근거 3종 세트

1. **WCET 분석**
   (Worst Case Execution Time)
2. **우선순위 기반 스케줄링**
3. **측정 결과 (Logic Analyzer, Trace)**

이 3개가 있으면 “이 시스템은 10ms deadline을 보장한다”라고 말할 수 있음.

---
> “RTOS에서 실시간성을 보장한다는 것은 태스크와 인터럽트의 **최악 지연 시간과 실행 시간의 상한을 계산할 수 있다는 의미**입니다.”
